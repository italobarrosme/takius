...
import { toResult } from '@/utils/errorHandler/toResult/toResult'
import { toNextResponse } from '@/utils/errorHandler/toResult/adapter'
...
export async function POST(request: Request) {
  ...
  const classType = body.class || 'Rogue'
  const description = body.description || ''
  const sprite = body.sprite || null
  ...
  const spriteImageUrl = sprite.imageUrl
  ...
  const prediction = await replicate.predictions.create({
    ...
    input: {
      prompt: `Create a detailed anime-style illustration based on this description: ${spriteDescription}. 
      The character must be ${sex}. Style: ${style}. Class: ${classType}. 
      Additional Description: ${description}. 
      Maintain the character's essence and design elements from the Ragnarok Online aesthetic.`,
      cfg_scale: 7,
      ...
      high_noise_frac: 0.8,
      negative_prompt: 'blurry, low quality, distorted, unrealistic, deformed',
    },
  })
